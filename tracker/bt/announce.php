...
//hardkor fix
if($ip == "172.16.0.73")$ip = "109.124.0.77";
//
...
// VIP
	...
	$vrow = DB()->fetch_row("
		...
		LIMIT 1
	");
	if ($vrow['vip_tarif'] > 0 AND $vrow['vip_ballance'] >=0 AND $vrow['vip_start_date'] < $ctime AND $vrow['vip_end_date'] > $ctime OR $vrow['vip_end_date'] < 0 AND $vrow['vip_lock'] < 1)
	{
		$vip = true;
	}
