...
if ($userdata['user_readonly'])
{
	...
	$user_readonly_array = DB()->fetch_row("SELECT moderator, expire, reason FROM `readonly_active` WHERE `user` = ".$userdata['user_id']." AND `active` = 1 LIMIT 1");
	$user_readonly_moderator = get_userdata($user_readonly_array['moderator']);
}
...
if ($t_data['topic_show_first_post'] && $start)
{
	$first_post = DB()->fetch_rowset("
		SELECT
		  u.username, u.user_id, u.user_timer, u.user_allow_viewonline,  u.user_posts, u.user_from, u.user_prov, u.user_status, u.vip_tarif, u.vip_ballance, u.vip_start_date, u.vip_end_date, u.vip_lock, u.user_gender,
		  u.user_regdate, u.user_rank, u.user_sig,
		  u.user_avatar, u.user_avatar_type,
		  u.user_opt, u.user_speed_up, u.user_speed_down,
		  p.*, u.user_readonly,
...
$template->assign_vars(array(
	...
	'PAGE_START'          => $start,
	'SHOW_JUMPBOX'        => true,
	'RO_ENABLED'          => $userdata['user_readonly'],
	'RO_MOD_ID'           => $userdata['user_readonly'] ? $user_readonly_array['moderator'] : false,
	'RO_MOD_NAME'         => $userdata['user_readonly'] ? $user_readonly_moderator['username'] : false,
	'RO_VIOLATION'        => $userdata['user_readonly'] ? $user_readonly_array['reason'] : false,
	'RO_DATE'             => $userdata['user_readonly'] ? date('d.m.Y', $user_readonly_array['expire']) : false,
...
if($bb_cfg['text_buttons']){
	if($postrow[$i]['user_gender'] == 0){
		$poster_gender = '';
	}elseif($postrow[$i]['user_gender'] == 1){
		$poster_gender = $lang['GENDER_SELECT'][1];
	}elseif($postrow[$i]['user_gender'] == 2){
		$poster_gender = $lang['GENDER_SELECT'][2];
	}
}else{
	if($postrow[$i]['user_gender'] == 0){
		$poster_gender = '';
	}elseif($postrow[$i]['user_gender'] == 1){
		$poster_gender = '<img src="images/gender_male.jpg" title="'.$lang['GENDER_SELECT'][1].'">';
	}elseif($postrow[$i]['user_gender'] == 2){
		$poster_gender = '<img src="images/gender_female.jpg" title="'.$lang['GENDER_SELECT'][2].'">';
	}
}
	...
	{
	$user_ro = '<img border="0" alt="Режим &quot;Только чтение&quot;" src="images/readonly.png">';
	}
	else
	{
	$user_ro = "";
		...
		if ($postrow[$i]['vip_tarif'] > 0 )
		{
			if ($postrow[$i]['vip_start_date'] < $cur_date)
			{
				if ($postrow[$i]['vip_end_date'] > $cur_date)
				{
					...
					{
						$vip_body = '<a href="vip_list.php"><img src="images/vip_user.png" title="VIP пользователь" alt="VIP пользователь"/></a>';
					}
					else
					{
						$vip_body = '';
	...
	// post rating
	//
	// display post ratings
	//
	if(!empty($postrow[$i]['post_rating'])){
		$post_rating = unserialize($postrow[$i]['post_rating']);
		$ratings = array(
				1 => 'Согласен',
				2 => 'Не согласен',
				3 => 'Смешно',
		...
		$ratings_count = array();
		$post_rating_html = "";
		foreach($post_rating as $post_rating_user_id => $post_rating_rating){
			@$ratings_count[$post_rating_rating]++;
		}
		if(count($ratings_count) <= 3) {
			foreach($ratings_count as $ratings_count_id => $ratings_count_times){
				$post_rating_html .= '<span><img src="images/postrating/'.$ratings_count_id.'.png" alt="'.$ratings[$ratings_count_id].'" /> '.$ratings[$ratings_count_id].' x <strong>'.$ratings_count_times.'</strong></span> ';
			}
		} else {
			foreach($ratings_count as $ratings_count_id => $ratings_count_times){
				$post_rating_html .= '<span><img src="images/postrating/'.$ratings_count_id.'.png" alt="'.$ratings[$ratings_count_id].'" /> x <strong>'.$ratings_count_times.'</strong></span> ';
			}
		}
	} else {
		$post_rating_html = "";
...
//Online/Offline
if ($postrow[$i]['user_timer'] >= ( time() - 300 ) && !bf($postrow[$i]['user_opt'], 'user_opt', 'allow_viewonline'))
{
$on_off_hidden = '<img src="' . $images['icon_online'] . '" alt="' . $lang['Online'] . '" title="' . $lang['Online'] . '" border="0" />';
}
else if (!bf($postrow[$i]['user_opt'], 'user_opt', 'allow_viewonline') == 0)
{
