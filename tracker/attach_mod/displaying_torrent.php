...
$upload_image              = '<img src="'.$bb_cfg['dn_icon_path'].'" alt="" border="0" />';
...
if ($tor_reged && $tor_info)
{
	...
	else
	{
		...
		$template->assign_block_vars('postrow.attach.tor_reged', array(
			...
			'TOR_TIME_SET'    => bb_date($tor_info['tor_type_time'], 'm d Y H:i:s', $userdata['user_timezone']), 
			'TOR_TIME_SET1'  => bb_date($tor_info['tor_type_time'], $bb_cfg['post_date_format'], $userdata['user_timezone']),
			...
			'TH_BUTTON'        => ($thanked || ($userdata['user_id'] == $poster_id) ? '' : '<span id="VB'.$attach_id.'">'. '<img src="images/sps.gif" onClick="this.disabled=true;ajax_do(\''
