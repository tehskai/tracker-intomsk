...
$post_new_release_url = "posting.php?mode=newrelease&amp;f=$forum_id";
...
#   echo "</pre>";                     #
########################################
#      К foreach приминимо break;      #
########################################

########################################
#      Release Button hide system      #
########################################
...
$rel_img = "./templates/tracker/images/lang_russian/release.gif";

foreach ($fid as $cat => $ids)
{
	if ($forum_id == $ids) 
	{
		$rel_button  = "<a href='$post_new_release_url'><img src='$rel_img' alt='Новый релиз' title=".$lang['POST_A_NEW_RELEASE']." /></a></td>";
		break;
	}else{
		$rel_button  = '';
	}
...
foreach ($topic_rowset as $topic)
{
	...
	if ($tr_cfg['gold_silver_enabled'] && isset($topic['tor_type']))
	{
		...
		{
			$is_gold = '<img src="images/tor_gold.gif" width="33" height="16" title="'.$lang['GOLD'].'" alt="" />&nbsp;';
		}
		elseif ($topic['tor_type'] == TOR_TYPE_SILVER)
		{
			$is_gold = '<img src="images/tor_silver.gif" width="33" height="16" title="'.$lang['SILVER'].'" alt="" />&nbsp;';
		}
	}
