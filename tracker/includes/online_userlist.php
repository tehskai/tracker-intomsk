...
foreach (DB()->fetch_rowset($sql) as $u)
{
	if ($u['session_logged_in'])
	{
		...
		else if ($level == GROUP_MEMBER && $u['user_rank'] == VIP) 
		{
		$name = "<b>$name</b>";
		$users_cnt['vip']++;
		$level = VIP;
		}
		else if ($level == GROUP_MEMBER && $u['user_rank'] == S_G) 
		{
		$name = "<b>$name</b>";
		$users_cnt['s_g']++;
		$level = S_G;
		}
		else if ($level == GROUP_MEMBER && $u['user_rank'] == EDIT_NVS) 
		{
		$name = "<b>$name</b>";
		$users_cnt['edit_nvs']++;
		$level = EDIT_NVS;
		}
		else if ($level == GROUP_MEMBER && $u['user_rank'] == RG_REALESERS) 
		{
		$name = "<b>$name</b>";
		$users_cnt['rg_realesers']++;
		$level = RG_REALESERS;
		}
		else if ($level == GROUP_MEMBER && $u['user_rank'] == RG_GAMEMAN) 
		{
		$name = "<b>$name</b>";
		$users_cnt['rg_gameman']++;
		$level = RG_GAMEMAN;
		}
...
[
	...
	<span class="colorVIP bold">{$users_cnt['vip']}</span> <span class="small">&middot;</span>
	<span class="colorNews bold">{$users_cnt['edit_nvs']}</span> <span class="small">&middot;</span>
	<span class="colorS_G bold">{$users_cnt['s_g']}</span> <span class="small">&middot;</span>
	<span class="colorRG_G bold">{$users_cnt['rg_gameman']}</span> <span class="small">&middot;</span>
