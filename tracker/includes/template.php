...
class Template {
	...
	function load_config($root, $edit_db)
	{
		...
		$this->cached_tpl_ext = $bb_cfg['xs_php'];
		$this->use_cache = $bb_cfg['xs_use_cache'];
		$this->auto_compile = $bb_cfg['xs_auto_compile'];
	...
	function set_filename($handle, $filename, $xs_include = false, $quiet = false)
	{
		...
		if(!empty($this->files_cache[$handle]) && !empty($bb_cfg['xs_auto_recompile']))
		{
			$cache_time = @filemtime($this->files_cache[$handle]);
			if(@filemtime($this->files[$handle]) > $cache_time || $bb_cfg['xs_template_time'] > $cache_time)
			{
...
#		$code = preg_replace('#\{(L_([a-z0-9\-_]+?))\}#i', '<'.'?php if (isset($L[\'$2\'])) { echo $L[\'$2\']; } else { echo ($V[\'$1\']) ? $V[\'$1\'] : $this->lang_error(\'$2\'); } ?'.'>', $code);
	...
	function xs_startup()
	{
		...
		{	// adding predefined variables
			...
			$tpl = $this->root . '/'; // BB_ROOT . 'templates/' . $this->tpl . '/';
			if(substr($tpl, 0, 2) === './')
			{
