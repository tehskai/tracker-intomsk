	...
	if ($mode == 'after_move')
	{
		...
		if (strlen($_POST['reason_move_bot']) > 0){
		  $reason_text = $lang['REASON_MOVE_BOT_MSG'].' '.$_POST['reason_move_bot']; //Filtrated XSS
				...
				$post_text = sprintf($lang['BOT_TOPIC_MOVED_FROM_TO'], "<a class=\"gen\" href=\"viewforum.php?f=$old_forum_id\">$forum_names[$old_forum_id]</a>", "<a class=\"gen\" href=\"viewforum.php?f=$forum_id\">$forum_names[$forum_id]</a>", htmlspecialchars($reason_text), profile_url($userdata));
