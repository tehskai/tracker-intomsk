...
{
	$message =  $lang['NULLRATIO1']. '<b>'. humn_size(MIN_DL_FOR_RATIO) . '</b><br /><br /><a href="' . append_sid("index.php?") . '">На главную</a>';
	message_die(GENERAL_MESSAGE, $message);
}	
else if ($ratio_nulled)
{
	$message =  $lang['NULLRATIO4']. '<br /><br /><a href="' . append_sid("index.php?") . '">На главную</a>';
	message_die(GENERAL_MESSAGE, $message);
}
else if (!$bb_cfg['rationull_enabled'])
{
	$message =  $lang['NULLRATIO9']. '<br /><br /><a href="' . append_sid("index.php?") . '">На главную</a>';
	message_die(GENERAL_MESSAGE, $message);
}
else if ($ratio > $bb_cfg['ratio_to_null'])
{
	$message =  $lang['NULLRATIO5']. '<br /><br /><a href="' . append_sid("index.php?") . '">На главную</a>';
	message_die(GENERAL_MESSAGE, $message);
}
else
{
...
{
	$sql = "UPDATE ". BB_BT_USERS ." SET u_up_total=0, u_down_total=0, u_up_release=0, u_up_bonus=0, ratio_nulled=1 WHERE user_id=". $user_id;
	DB()->sql_query($sql);
	$message =  $lang['NULLRATIO6'] . '<br /><br /><a href="' . append_sid("index.php?") . '">На главную</a>';
	message_die(GENERAL_MESSAGE, $message);
}
