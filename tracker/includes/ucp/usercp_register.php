...
foreach ($profile_fields as $field => $can_edit)
{
	...
	**/
	case 'user_status':
		$user_status = isset($_POST['user_status']) ? (string) $_POST['user_status'] : $pr_data['user_status'];
		$user_status = htmlCHR($user_status);
		if ($submit)
		{
			if ($user_status != $pr_data['user_status'])
			{
			$pr_data['user_status'] = $user_status;
			$db_data['user_status'] = (string) $user_status;
			}
		}
...
if($bb_cfg['new_user_reg_only_by_invite']){
	...
	*/
	else
	{
		...
		{
			bb_die($lang['NOTHING_HAS_CHANGED']); // . '<br /><br /><a href="' . PROFILE_URL . $userdata['user_id'] . '">'.$lang['RETURN_PROFILE'].'</a><br /><br />' . sprintf($lang['CLICK_RETURN_INDEX'],  '<a href="index.php">', '</a>')
		}
	}
}
