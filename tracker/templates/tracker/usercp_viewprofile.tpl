...
<var class="ajax-params">{action: "edit_user_profile", id: "user_status"}</var>
...
ajax.callback.group_membership = function(data) {
	$('#gr-mem-list').toggle().html(data.group_list_html);
}
</script>
<style type="text/css">
...
<div class="nav">
	...
	<!-- IF IS_ADMIN -->
	<p class="floatR">
	<a href="add_vip.php?v={PROFILE_USER_ID}" title="Сделать VIP">Настройки VIP <b>{USERNAME}</b></a> &middot;
...
<tr>
	<th colspan="2" class="thHead">{L_VIEWING_PROFILE} <!-- IF STATUS_PARK -->| {STATUS_PARK}<!-- ENDIF --></th>
</tr>
<tr>
	<td class="row1 vTop tCenter" width="30%">
		...
		<tr>
			...
			<td class="tLeft med" id="user_email">{EMAIL_IMG}</td>
		</tr>
		<!-- ENDIF -->
		<!-- IF PM -->
		<tr>
			<th>{L_PRIVATE_MESSAGE}:</th>
			<td class="tLeft med">{PM_IMG}</td>
		</tr>
		<!-- ENDIF -->
			<!-- IF WWW -->
			<tr>
				<th>{L_WEBSITE}:</th>
				<td class="tLeft med"><a href="{WWW}" class="editable">{WWW_IMG}</a></td>
			</tr>
			<!-- ENDIF -->
		<!-- IF ICQ -->
		<tr>
			<th>{L_ICQ}:</th>
			<!-- IF $bb_cfg['show_status_icons'] -->
			<td class="tLeft med" id="user_icq">
			<!-- ELSE -->
			<td class="tLeft med">
			<!-- ENDIF -->
				<span class="editable">{ICQ_IMG}<!-- IF $bb_cfg['show_status_icons'] -->{ICQ_STATUS_IMG}<!-- ENDIF -->
				</span>
			</td>
		</tr>
		<!-- ENDIF -->
		...
		<tr>
			...
			<td class="tLeft med">
			<!-- ENDIF -->
				<span class="editable">{SKYPE_IMG}<!-- IF $bb_cfg['show_status_icons'] -->{SKYPE_STATUS_IMG}<!-- ENDIF -->
		...
		<!-- ENDIF -->
			...
			<tr id="gr-mem-tr">
				...
				<td id="gr-mem-list1">
					<a href="#" class="med" onclick="ajax.group_membership('get_group_list'); return false;">{GROUP_MEMBERSHIP_TXT}</a>
				</td>
			</tr>
			<!-- ENDIF -->
		</table>
		<table cellpadding="2" cellspacing="0" border=0 class="borderless user_contacts w100" style="padding: 10px 10px 10px 10px;"><tr><td>&nbsp;</td><td><span id="gr-mem-list"></span></td></tr></table>
<!--/user_contacts-->
      <!--IFGROUPSW 
<a href="#"  class="med" onclick="ajax.group_membership('get_group_list'); return false;"><h4 class="cat border bw_TB" style="height:15px;"> {GROUP_MEMBERSHIP_TXT}</h4></a>
         ...
         1END1 1groups
         <tr>
            <td class="row2" nowrap="nowrap" align=left class="catLeft" id="gruoupsw"><span class="gentbl"><a href="{groups.U_GROUP_NAME}" class="gen"><b>{groups.L_GROUP_NAME}</b></a></span></td>
         </tr>
         </table>
         1ENDIF-->
		<!-- IF SIGNATURE -->
<h4 class="cat border bw_TB">{L_SIGNATURE_USER} <span class="editable bold">{USERNAME}</span></h4>
		<table class="nowrap borderless user_contacts w100">
			<tr>
			<td class="row1 pad_4">
	...
	<td class="row1" valign="top" width="70%">
		...
		<table class="user_details borderless w100">
			...
			<tr>
				...
				<td id="user_regdate">
					...
					<!-- ENDIF -->
					<!-- IF IS_AM -->[ <a href="#" class="med" onclick="ajax.mod_action('profile_ip'); return false;">{L_IP_ADDRESS}</a> ]<!-- ENDIF -->
				</td>
			</tr>
			<tr>
			...
			<tr>
				...
				<td>
					<p>
						<b>{POSTS}</b>&nbsp;
						[ <a class="med" href="{U_SEARCH_USER}" class="med">{L_SEARCH_USER_POSTS}</a> ]
						[ <a class="med" href="{U_SEARCH_TOPICS}" class="med">{L_SEARCH_USER_TOPICS}</a> ]
		...
		<tr>
			...
			<tr>
				...
				<td id="user_status">
					<span class="editable bold">{STATUS}</span>
				</td>
			</tr>
			<!-- ENDIF -->
					<tr>
...
<tr>
	...
	<!-- ELSE -->
	<!-- ENDIF -->
	<form action="{U_REPORT_USER}" method="post"><input type="submit" value="{L_REPORT_USER}" /></form>&nbsp;
