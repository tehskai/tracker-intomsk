...
<script type="text/javascript">
	$(document).ready(function(){ $('div.sp-head').click(); });</script>
<!-- ENDIF -->
<!-- ENDIF / LOGGED_IN -->
...
ajax.callback.user_stats = function(data){
  $('#user_stats_'+data.post_id+' p').html(data.html);
  f_list = true;
};
</script>
			...
			<p>
				...
				<label>
 					<input type="checkbox" <!-- IF SHOW_IMG_AFTER_LOAD -->{CHECKED}<!-- ENDIF -->
 						onclick="user.set('i_aft_l', this.checked ? 1 : 0);"/>{L_DOWNLOAD_PIC}
				</label>
			</p>
			</div>
			</fieldset>
...
<tr>
	...
	<th class="thHead td2" colspan="2">{L_MESSAGE}</th>
</tr>
...
<tr>
	...
	<!-- IF postrow.POSTER_BOT -->
		...
		<p class="avatar"><center>{postrow.POSTER_AVATAR}<center></p>
	<!-- ELSE -->
		<!-- IF QUICK_REPLY -->
		<p class="nick" title="{L_INS_NAME_TIP}" onmouseout="bbcode.refreshSelection(false);" onmouseover="bbcode.refreshSelection(true);" onclick="bbcode.onclickPoster('{postrow.POSTER_NAME_JS}');">
			<a href="#" onclick="return false;">{postrow.POSTER_NAME}</a> <!-- IF postrow.POSTER_AUTHOR --><sup>&reg;</sup><!-- ENDIF --><!-- IF postrow.READONLY --> {postrow.READONLY}<!-- ENDIF -->
		</p>
		<!-- ELSE -->
		<p class="nick">{postrow.POSTER_NAME} <!-- IF postrow.POSTER_AUTHOR --><sup>&reg;</sup><!-- ENDIF --><!-- IF postrow.READONLY --> {postrow.READONLY}<!-- ENDIF --></p>
		<!-- ENDIF -->
		...
		<!-- IF postrow.POSTER_AVATAR --><p class="avatar"><center>{postrow.POSTER_AVATAR}</center></p><!-- ENDIF -->
		<!-- IF postrow.POSTER_STATUS --><p class="status">{postrow.POSTER_STATUS}</p><!-- ENDIF -->
		<br>
		...
		<div>
				<p><span class="clickable small" style="margin: 5px 0 4px;" onclick="user_stats({postrow.POST_ID}, {postrow.POSTER_ID})"><b title="Нажмите что бы посмотреть">Подробнее</b></span></p>

			<span id="user_stats_{postrow.POST_ID}" style="display:none"><p><b>Загрузка...</p></span>
		</div>
	<!-- ENDIF -->
	...
	<td class="message td2" colspan="2">
		...
		<div class="post_head">
			...
			<p style="float: right;<!-- IF TEXT_BUTTONS --> padding: 3px 2px 4px;<!-- ELSE --> padding: 1px 6px 2px;<!-- ENDIF -->" class="post_btn_1">
				...
				<!-- IF AUTH_MOD -->
					...
					<a class="txtb" href="ban.php?mode=ban&amp;u={postrow.POSTER_ID}&amp;p={postrow.POST_ID}">{BAN_USER_IMG}</a>{POST_BTN_SPACER}
				<!-- ENDIF -->
			</p>
			<div class="clear"></div>
		</div>
...
<tr>
	...
	<script type="text/javascript">
   	...
   	function InitShowthread() {
	$('table#topic_main > tbody').each(function(){
        	$('#ratingcontrols_'+this.id).hide();
        	$(this).hover(function(){
            	$('#ratingcontrols_'+this.id).fadeIn(150);
        	},function(){
        		$('#ratingcontrols_'+this.id).fadeOut(150);
        	});
	});
	}
	function RatePost(postid, rating)
	{
    $.post('ajax.php', {'action': 'rate_post', postid: postid, rating: rating}, function(data){
        var $ratings = $('#rating_' + postid),
            $control = $('#ratingcontrols_post_' + postid),
            isError = (data.error != undefined),
            message = (isError) ? data.error : data.message;
        if (!isError) {
            $ratings.fadeOut('fast', function(){
                $ratings[0].innerHTML = data.result;
                $ratings.fadeIn('fast');
            });
        }
        $control.css({
        ...
        $control[0].innerHTML = message;
    }, 'json');
    $('#ratingslist_'+postid).remove();
    $('#ratingcontrols_post_' + postid)[0].innerHTML = "";
    return false;
}

	</script>
	<td style="padding-left:10px; text-align:left;"><div id="rating_{postrow.POST_ID}" class="rating_results">{postrow.POST_RATING}</div></td>
	<td style="padding-right:10px; text-align:right;"><div class="postrating" id="ratingcontrols_post_{postrow.POST_ID}">
	<a href="#" onclick="return RatePost( '{postrow.POST_ID}', '1');"><img src="images/postrating/1.png" alt="Agree" /></a> <a href="#" onclick="return RatePost( '{postrow.POST_ID}', '2');"><img src="images/postrating/2.png" alt="Disagree" /></a> <a href="#" onclick="return RatePost( '{postrow.POST_ID}', '3');"><img src="images/postrating/3.png" alt="Funny" /></a> <a href="#" onclick="return RatePost( '{postrow.POST_ID}', '4');"><img src="images/postrating/4.png" alt="Winner" /></a> <a href="#" onclick="return RatePost( '{postrow.POST_ID}', '5');"><img src="images/postrating/5.png" alt="Zing" /></a> <a href="#" onclick="return RatePost( '{postrow.POST_ID}', '6');"><img src="images/postrating/6.png" alt="Informative" /></a> <a href="#" onclick="return RatePost( '{postrow.POST_ID}', '7');"><img src="images/postrating/7.png" alt="Friendly" /></a> <a href="#" onclick="return RatePost( '{postrow.POST_ID}', '8');"><img src="images/postrating/8.png" alt="Useful" /></a> <a href="#" onclick="return RatePost( '{postrow.POST_ID}', '9');"><img src="images/postrating/9.png" alt="Optimistic" /></a> <a href="#" onclick="return RatePost( '{postrow.POST_ID}', '10');"><img src="images/postrating/10.png" alt="Artistic" /></a> <a href="#" onclick="return RatePost( '{postrow.POST_ID}', '11');"><img src="images/postrating/11.png" alt="Late" /></a> <a href="#" onclick="return RatePost( '{postrow.POST_ID}', '12');"><img src="images/postrating/12.png" alt="Dumb" /></a>
	</div>
...
<script type="text/javascript">InitShowthread();</script>