...
<h1 class="pagetitle">{PAGE_TITLE}</h1>
	<div id="whote_park" title="{L_PARK_PROFILE_STATUS_WHOTE}">
		<strong>Парковка аккаунта</strong><br />
		Предназначена для того что бы вы могли неограниченное количество времени не посещать торрент трекер.<br />
		Информация о парковке аккаунта:
		<ul>
			<li>При длительном отсутствии активности вашего аккаунта на трекере он не будет удален</li>
			<li>Если аккаунт припаркован скачивать торренты запрещено и вы не сможете ничего скачать</li>
		</ul>
   </div>
...
<!-- ELSE -->
<tr>
	<td>Код инвайта:<!-- IF $bb_cfg['new_user_reg_only_by_invite'] --> *<br /><small>Внимание!<br />На данный момент регистрация разрешена только по инвайтам.<br />Если вы не введете код инвайта, вы не сможете зарегистрироваться.</small><!-- ELSE --><br /><small>На данный момент разрешена регистрация без инвайтов.</small><!-- ENDIF --></td>
...
<tr>
    <td>{L_PARK_CONTROL}:
    <h6><a id="whote_link" href="#">{L_PARK_PROFILE_STATUS_WHOTE}</a></h6></td>
    <td>
...
<tr>
	<td>{L_PROV}:</td>
	<td>
		<input type="text" name="user_prov" size="50" value="{USER_PROV}" />
	</td>
</tr>
<tr>
	<td>{L_STATUS}:</td>
	<td>
		<input type="text" name="user_status" size="50" value="{USER_STATUS}" />
	</td>
</tr>
<!-- ENDIF -->
