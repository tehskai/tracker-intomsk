...
// Улучшение оформления раздач
   $mymymy = $userdata['user_id'];
    ...
    $result44 = DB()->sql_query("SELECT topic_id FROM bb_bt_torrents WHERE poster_id=$mymymy AND tor_status=5");
    $num_rows44 = DB()->num_rows($result44);
    // Кол-во неоформленных релизов
    $result55 = DB()->sql_query("SELECT topic_id FROM bb_bt_torrents WHERE poster_id=$mymymy AND tor_status=4");
    $num_rows55 = DB()->num_rows($result55);
    // Кол-во закрытых релизов
    $result66 = DB()->sql_query("SELECT topic_id FROM bb_bt_torrents WHERE poster_id=$mymymy AND tor_status=1");
    $num_rows66 = DB()->num_rows($result66);
    ...
    while($row44 = DB()->sql_fetch_array($result44))
    {
    $bbuf = $row44[0];
    $query_buffer = DB()->sql_query("SELECT topic_title FROM bb_topics WHERE topic_id=$bbuf");     
    $row_buf = DB()->sql_fetch_array($query_buffer);   
    ...
    while($row55 = DB()->sql_fetch_array($result55))
    {
    $bbuf = $row55[0];
    $query_buffer = DB()->sql_query("SELECT topic_title FROM bb_topics WHERE topic_id=$bbuf");     
    $row_buf = DB()->sql_fetch_array($query_buffer);   
    ...
    while($row66 = DB()->sql_fetch_array($result66))
    {
    $bbuf = $row66[0];
    $query_buffer = DB()->sql_query("SELECT topic_title FROM bb_topics WHERE topic_id=$bbuf");     
    $row_buf = DB()->sql_fetch_array($query_buffer);   
...
if (!$is_auth[$is_auth_type])
{
	...
	switch ($mode)
	{
		case 'newtopic':
			...
			break;
		case 'newrelease':
			$redirect = "mode=newrelease&amp;f=$forum_id";
			break;
		case 'reply':
...
if ($mode == 'newrelease' && $topic_tpl && $post_info['topic_tpl_id'])
{
	if($tor_status = join(',', $bb_cfg['tor_cannot_new']))
    {
		$sql = DB()->fetch_rowset("SELECT t.topic_title, t.topic_id, tor.tor_status
...
else
{
	...
	if (!IS_GUEST && $mode != 'newrelease' && !$notify_user)
	{
		$notify_user = (int) DB()->fetch_row("
			SELECT topic_id
			FROM ". BB_TOPICS_WATCH ."
...
if (!IS_GUEST && $mode != 'newrelease' && ($submit || $preview || $mode == 'quote' || $mode == 'reply') && isset($_COOKIE[COOKIE_TOPIC]))
{
	if ($topic_last_read = max(intval(@$tracking_topics[$topic_id]), intval(@$tracking_forums[$forum_id])))
	{
		$sql = "SELECT p.*, pt.post_text, u.username, u.user_rank
...
//snp end
{
	...
	if (!$error_msg)
	{
		...
		{
			$user_id = ( $mode == 'reply' || $mode == 'newtopic' || $mode == 'newrelease') ? $userdata['user_id'] : $post_data['poster_id'];
...
else
{
	...
	else if ( $mode == 'newrelease' )
	{
		...
		$subject = '';
		$message = '';
include($phpbb_root_path .'release_category.php');
foreach ($release as $category => $release)
{
	$rel = explode(",", $release);
	foreach ($rel as $release)
	{
		if ($forum_id == $release) header ("Location: release.php?what=".$category."&fid=".$forum_id);
	}
}
	}
	else if ( $mode == 'reply' )
...
switch( $mode )
{
	...
	case 'newrelease':
		$page_title = $lang['POST_A_NEW_RELEASE'];
		$hidden_form_fields .= '<input type="hidden" name="' . POST_FORUM_URL . '" value="' . $forum_id . '" />';
		break;
...
$template->assign_vars(array(
	...
	'POSTING_RELEASE_ID' => ($mode != 'newrelease') ? $topic_id : '',
	'POSTING_RELEASE_TITLE' => ($mode != 'newrelease') ? '' : '',
...
if ($mode == 'newrelease' || $post_data['first_post'])
{
	$template->assign_var('POSTING_SUBJECT');
}
