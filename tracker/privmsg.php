...
else
{
	...
	$sql_tot = "SELECT COUNT(privmsgs_id) AS total
		FROM " . BB_PRIVMSGS . " ";
	$sql = "SELECT pm.privmsgs_type, pm.privmsgs_id, pm.privmsgs_date, pm.privmsgs_subject, u.user_id, u.username, u.user_avatar, u.user_avatar_type, u.user_rank
		FROM " . BB_PRIVMSGS . " pm, " . BB_USERS . " u ";
	switch( $folder )
	{
		case 'inbox':
	...
	if ( $row = DB()->sql_fetchrow($result) )
	{
		...
		$msg_avatar = ( $bb_cfg['show_no_avatar'] ) ? '<img width=40 height=40 src="' . $bb_cfg['no_avatar'] . '" alt="" border="0" />' : '';
		if ( $row['user_avatar_type'] )
		{
			switch( $row['user_avatar_type'] )
			{
				...
				case USER_AVATAR_REMOTE:
					$msg_avatar = ( $bb_cfg['allow_avatar_remote'] ) ? '<img src="' . $row['user_avatar'] . '" height="40" alt="" border="0" />' : '';
					break;
				case USER_AVATAR_GALLERY:
					$msg_avatar = ( $bb_cfg['allow_avatar_local'] ) ? '<img src="' . $bb_cfg['avatar_gallery_path'] . '/' . $row['user_avatar'] . '" height="40" alt="" border="0" />' : '';
					break;
			}
		}
